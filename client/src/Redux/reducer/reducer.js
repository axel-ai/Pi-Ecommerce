...
const rootReducer = (state = initialState, action = {}) => {
  ...
  switch (action.type) {
    ...
    case FILTER_BRAND:
      ...
      const marca = filteredBrands[0].brands;
      return {
        ...state,
        marcacel: marca,
    ...
    case FILTER_DISPLAY:
      ...
      const filteredBrands2 =
        state.marcacel === 'All'
          ? state.allProducts
          : allPhones2.filter((p) => p.brands === state.marcacel);

      let size = {};

      const producto = state.products;

      if (action.payload === 'All') {
        size = filteredBrands2;
      }
      if (action.payload === 'menor-display') {
        size = filteredBrands2?.filter((e) => {
          return e.display < 6.3;
        });
      }
      if (action.payload === 'entre-display') {
        size = filteredBrands2?.filter((e) => {
          return e.display > 6.2 && e.display < 6.6;
        });
      }
      if (action.payload === 'mayor-display') {
        size = filteredBrands2?.filter((e) => {
          return e.display > 6.5;
        });
      }
      return {
        ...state,
